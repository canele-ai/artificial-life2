"""solution.py — Flow-Lenia × Sep-CMA-ES × CLIP open-endedness (orbit 15).

eval-v2 RE-RUN of orbit 09 (which inherited orbit 03's hypothesis).

Provenance story
----------------
Orbit 09 (`09-flow-lenia-real-cmaes`, issue #10) re-submitted orbit 03's
Flow-Lenia hypothesis on eval-v2, but CRASHED inside the Modal search
container with `preprocessor_config.json` missing from `/cache/hf`.  Root
cause: `search_container` had `block_network=True`, which prevented the
CLIP cache from refetching that file when the mounted Volume was incomplete.
The research question — axis 1 substrate gap: does mass-conserving
Flow-Lenia beat plain Lenia on the 5-tier judge? — was therefore never
actually tested on eval-v2.

eval-v2 + infra fix (commit 08e6131) dropped `block_network=True` on
`search_container`, so the HuggingFace cache can now repopulate any missing
files from the CLIP snapshot at runtime.  This orbit re-submits orbit 09's
solution verbatim (the code was correct; only infra was broken).
...
def search(
    ...
    return {
        ...
        "search_trace": {
            ...
            "algorithm": "sep_cma_es_flow_lenia_eval_v2_infra_fixed",
